...
//        }
	...
	void SurfaceMesh::collapseqem(Halfedge h, vec3 newv)
	{
		//let's make it sure it is actually checked
		assert(is_collapse_ok(h));
		std::vector<SurfaceMesh::Vertex> start_one_ring;   //定义一环邻域
		std::vector<SurfaceMesh::Vertex> end_one_ring;
		...
		SurfaceMesh::Vertex   start_v = source(h);          //找到边上的两个点
		SurfaceMesh::Vertex   end_v = target(h);
		allone_ring.clear();
		face_points.clear();
		//start_one_ring.clear();                            //清理
		//end_one_ring.clear();
		//for (auto vv : this->vertices(start_v)) start_one_ring.push_back(vv);
		//for (auto bb : this->vertices(end_v)) end_one_ring.push_back(bb);
		for (auto vv : this->vertices(start_v)) allone_ring.push_back(vv);
		for (auto bb : this->vertices(end_v)) {             //对每个bb
			int i = 0;
		...
		/*用边和循环来删除
		for (auto h : this->halfedges(start_v)) {
			remove_edge(h);
			if (next(next(h)) == h) { remove_loop(h); }
		}
		for (auto hh : this->halfedges(end_v)) {
				remove_edge(hh);
				if (next(next(hh)) == hh) { remove_loop(hh); }
		}*/
		//for (int i = 0; i < allone_ring.size(); ++i) std::cout << "all_allone_ring:" << allone_ring[i] << "\n";

		//this->delete_vertex(start_v);              //删除半边的两个点
		//this->delete_vertex(end_v);


		Halfedge h0 = h;           //需要塌陷
		Halfedge h1 = prev(h0);    //返回事件面内的前半边//需要塌陷
		Halfedge o0 = opposite(h0);//需要塌陷
		Halfedge o1 = next(o0);    //需要塌陷


		Vertex vh = target(h);  //最终保留的顶点，边v0v1中的v1
		vpoint_[vh] = newv;
		// remove edge
		remove_edge(h0);
