...
void MeshFilteringBF::BilateralNormalFilter(int itr)
{
...
/*-----------------------寻找面的二邻域------------------------*/
		...
		{
			if (dot(fnormal_[f], fnormal_[*or_it]) > cos(20 * M_PI / 180))
			{
...
void MeshFilteringBF::vertexUpdate(int itr, double specified_threshold)
{
...
void MeshFilteringBF::Runfilter(int iteration, double specified_threshold)
{
	...
	{
		BilateralNormalFilter(iteration);
		cout << "end normal filter:" << iteration << endl;
		vertexUpdate(iteration, specified_threshold);
		cout << "end vertex update:" << iteration << endl;
	}
}
