...
ViewerQt::ViewerQt(QWidget *parent /* = nullptr*/)
	...
	//像Qt::StrongFocus加上小部件通过使用鼠标滚轮接受焦点。
...
ViewerQt::~ViewerQt() {
	...
	//确保上下文是当前的，然后显式地销毁所有底层的OpenGL资源。
	makeCurrent();//确认环境

	cleanup(); //OpenGL资源(例如，着色器，纹理，VAOs)必须在存在一个有效的渲染上下文时销毁。
...
void ViewerQt::postDraw() {
	...
	if (texter_ && texter_->num_fonts() >= 2) {
		...
		texter_->draw("MP", offset, offset, font_size, 0);

		// FPS computation  FPS计算
